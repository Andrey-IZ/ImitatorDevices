...
import logging
from tools_parse_yaml_protocol import str_hex2byte, load_handler, load_conf_test, str_dict_keys_lower


class FoldingSettings(object):
    """
    ...
    def serial_settings(self, settings):
        for opts in self.__serial_settings.keys():
            if settings.get(opts):
                self.__serial_settings[opts] = settings[opts]
            else:
                self.log.warning("Serial settings is redundant")

    def parse(self, file_name):
        self.__lists_protocol.clear()
        for i, cmd in enumerate(load_conf_test(file_name)[1:]):
            ...
            gen_list_resp = self.__generate_list_response(handler_response, list_req, order,
                                                          response, (doc, order))
            if isinstance(gen_list_resp, list) and len(list_req) != len(gen_list_resp):
    ...
    @staticmethod
    def __generate_list_response(handler_dict_response, list_req, order, response, param_info):
        list_resp = []
        if isinstance(handler_dict_response, dict):
            ...
            if order == 'generator':
                if isinstance(func_handler_response, list):
                    ...
                    return [func_handler_response], response
            elif order == 'zip' or order == 'packet':
                return FoldingSettings.__genearate_list_packet(handler_dict_response, response, param_info)
            ...
            if hasattr(generator_resp, '__call__'):  # it responses function-generator
                ...
                while self.__count_req_generator_packet < len_list_req:
                    ...
                    if packet_response[0] == bytes_recv:
                        ...
                        self.log.info('!INFO: Handling command: "{0}", function = "{2}",'
                                      ' order = {1}'.format(doc, order, generator_resp.__name__))
                        self.log.debug(u'!DEBUG: Response generate packet count = {0} of {1}'.format(
                            self.__count_req_generator_packet + 1, len_list_req))
    ...
    def __process_zip_response(self, param_data, param_info):
        ...
        while self.__count_req_zip_packet < len_list_req:
            ...
            if packet_request == bytes_recv:
                ...
                self.log.info('!INFO: Handling command: "{0}"'
                              ' order = {1}'.format(doc, order))
                self.log.debug(u'!DEBUG: Response generate packet№ = {0} from {1}'.format(
                    self.__count_req_zip_packet + 1, len_list_req))
    ...
    def handler_response(self, bytes_recv: "received bytes array") -> bytes:
        """
        ...
        while self.__count_protocol < self.__len_list_porotocol:
            ...
            list_req = cmd[1]
            doc = cmd[3]
            self.log.warning("***  seek ---- order = {}, doc = {}".format(order, doc))
            if order == "generator" and self.__count_req_generator_packet >= 0 and \
                            self.__count_req_zip_packet == 0 and self.__count_req_packet == 0:
                list_generator_resp, response = cmd[2]
                self.log.warning("======>  INTO {}, doc = {}, count = {}".format(order, doc, self.__count_protocol))
                ...
                if result:
                    return result
                            ...
                            self.__count_req_generator_packet == 0 and self.__count_req_packet == 0:
                list_resp = cmd[2]
                self.log.warning("======>  INTO {}, doc = {}, count = {}".format(order, doc, self.__count_protocol))
                result = self.__process_zip_response((list_resp, list_req, bytes_recv), (doc, order))
                if result:
                    return result
            elif order == "packet" and self.__count_req_packet >= 0 and \
                            self.__count_req_generator_packet == 0 and self.__count_req_zip_packet == 0:
                list_resp = cmd[2]
                self.log.warning("======>  INTO {}, doc = {}, count = {}".format(order, doc, self.__count_protocol))
                result = False  # self.__process_zip_response((list_resp, list_req, bytes_recv), (doc, order))
                if result:
                    return result

            if self.__count_protocol == self.__len_list_porotocol - 1:
                if self.__count_req_packet == 0 and \
                            self.__count_req_generator_packet == 0 and self.__count_req_zip_packet == 0:
                    self.log.warning(
