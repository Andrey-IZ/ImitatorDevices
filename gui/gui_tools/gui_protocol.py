...
class GuiProtocol(object):
    def __init__(self, logger, main_window):
        ...
        self.__count_tags = {KW_GROUPS: 0, KW_FIELDS: 0, KW_PAGES: 0}
    ...
    def append(self, dict_gui_property):
        ...
        for page in dict_gui_property.get(KW_PAGES, []):
            ...
            self.__count_tags[KW_PAGES] += 1
            page['id'] = self.__count_tags[KW_PAGES]
            dict_page.update(page)

            dict_group = {}
            for group in dict_page.get(KW_GROUPS, {}):
                copy_group = copy.copy(self.__default_group_dict)
                self.__count_tags[KW_GROUPS] += 1
                group['id'] = self.__count_tags[KW_GROUPS]
                ...
                for field in copy_group.get(KW_FIELDS, {}):
                    if field.get(KW_GUI_CONTROL_TYPE) == KW_FIELD_SPINBOX:
                        ...
                        raise ValueError('Found unrecognized type')

                    self.__count_tags[KW_FIELDS] += 1
                    field['id'] = self.__count_tags[KW_FIELDS]
            ...
            dict_page[KW_GROUPS] = dict_group
            self.__dict_gui_ctrl[dict_page.get(KW_GUI_NAME)] = dict_page
        return self.__dict_gui_ctrl

    def build_form(self):
        self.__central_layout = self._init_tabs()
        for page in sorted(self.__dict_gui_ctrl.values(), key=lambda x: x['id']):
            _page = self._add_page(page.get(KW_GUI_NAME))
            page[KW_GUI_REF] = _page[0]
            page[KW_GUI_HLAYOUT] = _page[1]
            page[KW_GUI_VLAYOUT] = _page[2]

            for group in sorted(page.get(KW_GROUPS, {}).values(), key=lambda x: x['id']):
                ...
                for field in sorted(group.get(KW_FIELDS, {}).values(), key=lambda x: x['id']):
                    if field.get(KW_GUI_CONTROL_TYPE) == KW_FIELD_SPINBOX:
        ...
        for i in self.__count_tags:
            self.__count_tags[i] = 0
